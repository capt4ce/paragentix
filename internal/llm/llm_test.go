...
func TestOpenAICompatibleSendsToolsAndParsesToolCalls(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/chat/completions" || r.Header.Get("Authorization") != "Bearer secret" { t.Fatalf("bad request: %s %s", r.URL.Path, r.Header.Get("Authorization")) }
	...
	if len(res.ToolCalls) != 1 || res.ToolCalls[0].Name != "file_read" { t.Fatalf("bad response: %+v", res) }
}

func TestNewProviderNormalizesDeepSeekAndOpenAICompatible(t *testing.T) {
	if _, ok := NewProvider(config.ModelConfig{Provider: "deepseek"}).(OpenAICompatible); !ok { t.Fatal("deepseek should use OpenAI-compatible adapter") }
	if _, ok := NewProvider(config.ModelConfig{Provider: "openai-compatible"}).(OpenAICompatible); !ok { t.Fatal("openai-compatible should use OpenAI-compatible adapter") }
}
...
func TestCodexProviderUsesConfiguredCommand(t *testing.T) {
	...
	if err != nil || strings.TrimSpace(res.Content) != "ok" { t.Fatalf("res=%+v err=%v", res, err) }
}
