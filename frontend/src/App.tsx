...
export function App() {
  ...
  {view==='board'&&<main className="board">{cols.map(c=><section className="lane"><div className="lanehead"><b>{c.name}</b><button onClick={async()=>{setForm({...c,projects:await api(`/workspaces/${board.workspaceId}/projects`)});setDialog('edit column')}}>Edit</button></div><small>Project: {c.projectName}{c.worktreeEnabled?` · Worktree: ${c.worktreeName}`:''}</small>{c.jobs?.map((j:any)=><article key={j.id} tabIndex={0} role="button" onClick={()=>setJob(j)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setJob(j)}}} className={'job '+j.state}><b>{j.task}</b><StatusBadge state={j.state}/>{j.cli_tool&&<small>{j.cli_tool}</small>}</article>)}<button className="add" onClick={()=>{setForm({columnId:c.id,task:'',doneDefinition:''});setDialog('job')}}>+ Add job</button></section>)}{board&&<button className="newlane" onClick={async()=>{const projects=await api(`/workspaces/${board.workspaceId}/projects`);setForm({projects,projectId:projects[0]?.id,worktreeEnabled:false});setDialog('column')}}>+ Add column</button>}</main>}
