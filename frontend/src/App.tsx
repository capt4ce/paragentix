...
export const swipedColumn = (current: number, count: number, deltaX: number) =>
  Math.max(0, Math.min(count - 1, current + (deltaX < -50 ? 1 : deltaX > 50 ? -1 : 0)));
function JobDetail({
  job,
  close,
  refresh,
...
export function App() {
  ...
  {view==='board'&&<main className="board"><div className="column-tabs" role="tablist" aria-label="Columns">{cols.map((c,i)=><button key={c.id} role="tab" aria-selected={activeColumn===i} onClick={()=>setActiveColumn(i)}>{c.name}</button>)}{board&&<button className="add-column-tab" onClick={async()=>{const projects=await api(`/workspaces/${board.workspaceId}/projects`);setForm({projects,projectId:projects[0]?.id,worktreeEnabled:false});setDialog('column')}}>+ Add column</button>}</div>{cols[activeColumn]&&(()=>{const c=cols[activeColumn];return <section className="lane" role="tabpanel" onTouchStart={e=>touchX.current=e.changedTouches[0].clientX} onTouchEnd={e=>setActiveColumn(swipedColumn(activeColumn,cols.length,e.changedTouches[0].clientX-touchX.current))}><div className="lanehead"><b>{c.name}</b><span className="lane-actions"><button aria-label={`Edit ${c.name}`} title="Edit column" onClick={async()=>{setForm({...c,projects:await api(`/workspaces/${board.workspaceId}/projects`)});setDialog('edit column')}}><Pencil size={16}/></button><button className="danger" aria-label={`Archive ${c.name}`} title="Archive column" onClick={async()=>{await archiveColumn(c.id);setActiveColumn(Math.max(0,activeColumn-1));await load()}}><Archive size={16}/></button></span></div><small>Project: {c.projectName}{c.worktreeEnabled?` · Worktree: ${c.worktreeName}`:''}</small>{c.jobs?.map((j:any)=><article key={j.id} tabIndex={0} role="button" onClick={()=>setJob(j)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setJob(j)}}} className={'job '+j.state}><b>{j.task}</b><StatusBadge state={j.state}/>{j.cli_tool&&<small>{j.cli_tool}</small>}</article>)}<button className="add" onClick={()=>{setForm({columnId:c.id,task:'',doneDefinition:''});setDialog('job')}}>+ Add job</button></section>})()}</main>}
