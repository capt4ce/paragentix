...
export function JobCard({ job, open, archive }: { job: any; open: () => void; archive: () => Promise<void> }) {
  return <article className={'job '+job.state}><button type="button" className="job-open" onClick={open}><b>{job.task}</b><StatusBadge state={job.state}/>{job.cli_tool&&<small>{job.cli_tool}</small>}</button><button type="button" className="job-archive danger" aria-label={`Archive ${job.task}`} title="Archive job" onClick={async e=>{e.stopPropagation();await archive()}}><Archive size={16}/></button></article>;
}
function JobDetail({
  job,
  close,
